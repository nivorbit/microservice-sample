...
local plugin_name = "nivorbit-openid-connect"
...
local schema = {
    ...
    required = {"client_id", "client_secret", "discovery", "token_exchange", "token_endpoint", "audience"}
}
...
local function exchange_token(ctx, conf, token)
    if token then
        -- Exchange current token with new one
        local body = {
        ...
        local json, err = openidc.call_token_endpoint(conf, conf.token_endpoint, body, conf.introspection_endpoint_auth_method)
        if err then
            core.log.warn("token exchange failed... returning original token")
            core.log.error(err)
            return token
        end
...
local function add_access_token_header(ctx, conf, token)
    if token then
        ...
        if conf.token_exchange then
            core.log.warn("Exchanging token")
            access_token = exchange_token(ctx, conf, token)
        end

        if conf.set_access_token_header then
            if conf.access_token_in_authorization_header then
                if not core.request.header(ctx, "Authorization") or conf.token_exchange then
                    -- Add Authorization header.
                    core.request.set_header(ctx, "Authorization", "Bearer " .. access_token)
                end
            else
                if not core.request.header(ctx, "X-Access-Token") or conf.token_exchange then
                    -- Add X-Access-Token header.
                    core.request.set_header(ctx, "X-Access-Token", access_token)
                end
            end
        end
    end
